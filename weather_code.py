...
class Weather:
    def __init__(self, latitude, longitude, api_id):
        ...
        self.data = requests.get(
            f"https://api.openweathermap.org/data/2.5/onecall?lat=[your_lat]&lon=[your_long]&lang=fr&appid=[you_Weather_API]").json()
...
class Pollution:
    ...
    def update(self, lattitude, longitude, api_id):
        self.data = requests.get(
            f"http://api.openweathermap.org/data/2.5/air_pollution?lat=[your_lat]&lon=[your_long]&lang=fr&appid=[you_Weather_API]").json()
        return self.data
